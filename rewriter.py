...
class query_rewrite:
    ...
    def page(self, expression, is_union=False, level=0):
        ...
        if expression.args["from"].find(exp.Subquery):
            ...
            #       if self.contains_agg:
            #         self.is_rewritable = False
            #       self.contains_agg = True
            #       self.add_page_id(cte_expression, True, True, is_union)
            #   else:
            #       self.add_page_id(cte_expression, False, True, is_union)
            if "joins" in expression.args:
                for join_expression in expression.args["joins"]:
...
[2, 3, 6, 9, 10, 13, 17, 19, 25,  29, 32, 33, 42, 43, 45, 48, 50, 52, 55, 61, 62, 66, 67, 76, 77, 80, 85, 86, 88, 90, 91, 92, 96, 97, 99]
for i in [97]:  # [1,4,5,6,7,8,9,12,14,17,19,22]:
    query_file = (
